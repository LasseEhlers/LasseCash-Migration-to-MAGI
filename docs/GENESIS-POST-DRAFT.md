...
<!-- Fill on launch day: {GENESIS_BLOCK} {CONTRACT_ID} {MERKLE_ROOT} {ACCOUNTS_IN} {ACCOUNTS_BURNED} {LC_MIGRATED} {LC_BURNED} {BLOCK_Y} {POOL_HBD} {DATE} -->

## What happened

At Hive block **109,504,918** (Monday 31 August 2026, 12:00 UTC) I took the snapshot I announced. It is final and it is public: **{ACCOUNTS_IN} accounts** holding **{LC_MIGRATED} LASSECASH** are in; **{ACCOUNTS_BURNED} accounts** holding **{LC_BURNED} LASSECASH** did not qualify and that amount now sits at @null, forever, listed account by account.

The whole snapshot is committed to the MAGI contract as one Merkle root:

`{MERKLE_ROOT}`

Contract id: `{CONTRACT_ID}` — genesis at Hive block **{GENESIS_BLOCK}**, {DATE}.
...
I opened the pool with **10,000 LASSECASH and {POOL_HBD} HBD** — the same ratio as the old Hive-Engine pool on migration day. That is deliberately small: the opening *ratio* is what matters, and arbitrage keeps any depth honest.
...
The owner key is destroyed at block **{BLOCK_Y}** (≈ day 40), after the migration mints have matured and the first full monthly Proof-of-Brain payout has settled. Until then the key can do exactly one thing: propose a code update. It cannot touch anyone's tokens. Every proposed update is visible on-chain for 48 hours before it can activate — query `findPendingContractUpdates` for contract `{CONTRACT_ID}` at `https://api.vsc.eco/api/v1/graphql` — and can be cancelled inside that window. After block {BLOCK_Y} no update can ever be proposed by anyone, including me. The burn transaction id will be published here.
